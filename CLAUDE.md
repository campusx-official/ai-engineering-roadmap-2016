...
A standalone marketing-grade website that presents the AI Engineering roadmap as a **visual journey through 14 levels (0–13)**. It is a sibling to the main CampusX site — same brand, calmer/more editorial tone — and its job is to (a) orient a learner on the whole path, (b) let them drill into any level, and (c) route them to the relevant CampusX One courses.
...
```
level0-software-fundamentals.txt      level7-llm-evals.txt
...
- `/` — **Roadmap overview**: the hero map of all 14 levels, grouped into phases, plus intro, "how to use", and a primary CampusX CTA.
...
- On each level, **map to relevant CampusX One catalog items** as "learn this on CampusX" cards. Use the real catalog; mark unreleased ones "coming soon". Example mapping (extend per level, confirm against live catalog):
  ...
  - L4 → Advanced RAG
  - L5 → LangGraph (notes), CrewAI, Agno, MCP (notes)
  - L6 → Context Engineering *(coming soon)*
  - L7 → LLM Evaluations *(coming soon)*
  - L8 → LLM Guardrails *(coming soon)*
  - L10 → LLMOps *(coming soon)*, Docker
  - L11 → DLCV
...
- **Deep-linkable + navigable** — clean `/level/[n]` routes, prev/next, and a **sticky level navigator** (mini-map) so learners never feel lost across 14 levels.
...
- All 14 levels parsed into typed data and rendered.
